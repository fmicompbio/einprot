#' Utility functions to check validity of arguments
#'
#' These functions have been largely copied from https://github.com/fmicompbio/monaLisa/blob/master/R/utils.R
#'
#' @keywords internal
#' @noRd
...
.assertVector <- function(x,
    ...
    if (!is.null(validValues) && !all(x %in% validValues)) {
        stop("'", xname, "' must be a subset of: ", paste(validValues,
                                                          collapse = ", "))
    }
