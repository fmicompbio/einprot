...
suppressPackageStartupMessages({
    ...
    library(GSEABase)
    # library(einprot)
    devtools::load_all("/work/gbioinfo/sonechar/Projects/pipeline_proteomics/combined_package/einprot")
})
options(cores = 2)
```
...
pd_contaminants <- getContaminantsDatabaseFrompdAnalysis(pdAnalysisFile)
...
quant_info <- getQuantInfoFrompdAnalysis(pdAnalysisFile)
pd_quant_mode <- quant_info$quant_mode
pd_abundance <- quant_info$abundance_based_on
pd_quanvaluecorrection <- quant_info$quan_value_corr
pd_peptides_for_quantification <- quant_info$peptides_to_use
pd_CoIsolationThr <- quant_info$co_isolation_thr
pd_avReporterSNThr <- quant_info$ave_reporter_sn_thr
pd_SPSMMpct <- quant_info$sps_mm_pct_thr
pd_normMode <- quant_info$norm_mode
pd_ImputationMode <- quant_info$imputation_mode
...
colsToRemove <- c(grep("Found.in.Fraction|Abundances.Grouped.|Abundances.Grouped.CV.in.Percent.|Abundances.Grouped.Count.|Abundances.Normalized.|Found.in.Sample.Group.|Found.in.Sample|Proteins.Unique.Sequence.ID", 
                       colnames(rowData(sce)), value = TRUE))
