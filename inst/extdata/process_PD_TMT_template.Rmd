...
if (inputLevel == "Proteins") {
    string_db <- tryCatch({
        ...
        warning("The STRINGdb object can not be created. No STRING analysis will be performed.", 
                call. = FALSE)
        NULL
    })
} else {
...
for (a in unique(c(aNames$assayImputed, aNames$assayBatchCorr))) {
    ...
    p <- ggplot(pcadf, aes(x = PC1, y = PC2, color = group, label = sampleLabel)) + 
        ...
        theme_cowplot()
    print(p + geom_encircle(aes(fill = group, group = group), alpha = 0.5, 
