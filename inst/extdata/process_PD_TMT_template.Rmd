...
```{r define-submission-list, echo=FALSE}
...
```{r get-exp-info, echo=FALSE, warning=FALSE}
...
## Proteome Discoverer analysis summary
...
rowData(qft[[1]])$Reverse[is.na(rowData(qft[[1]])$Reverse)] <- ""
rowData(qft[[1]])$Potential.contaminant[is.na(rowData(qft[[1]])$Potential.contaminant)] <- ""
rowData(qft[[1]])$Only.identified.by.site[is.na(rowData(qft[[1]])$Only.identified.by.site)] <- ""
...
if ("GO" %in% includeFeatureCollections) {
    # up <- UniProt.ws(taxId = tax_id)
    # goannots <- AnnotationDbi::select(
    #     up, keys = rowData(qft[[1]])$Accession, 
    #     keytype = "UNIPROTKB_ID", columns = "GO"
    # ) %>%
    #     tidyr::separate_rows(GO, sep = "; ")
    # goannots <- as(lapply(split(goannots, f = goannots$GO), 
    #                       function(w) unique(w$UNIPROTKB_ID)),
    #                "CharacterList")
...
will be extracted from the sample name in the `Proteome Discoverer` file. 
The `group` column will be used to define groups for the statistical testing later. 
...
Next, we filter out any features classified by Proteome Discoverer as potential 
contaminants. 
...
```{r visualize-distributions-imputed, fig.width=7, fig.height=7, message = FALSE}
...
```{r run-limma, echo=(stattest=="limma"), eval=(stattest=="limma")}
for (cmp in comparisons) {
    ...
    for (setname in names(featureCollections)) {
        tmpres <- as.data.frame(mcols(featureCollections[[setname]]), optional = TRUE) %>%
    ...
    ## Get abundance values to add to result table (the ones that were used for the analysis)
...
```{r run-ttest, echo=(stattest=="ttest"), eval=(stattest=="ttest")}
for (cmp in comparisons) {
    ## Subset QFeatures object to only the two groups considered
    qftsub <- qft[, qft$group %in% cmp]
...
for (nm in names(tests)) {
    ...
    ggbase <- ggplot(tests[[nm]], aes(x = .data[[xv]], y = .data[[yv]])) + 
    ...
    gg0 <- ggbase + geom_point(fill = "lightgrey", color = "grey", pch = 21, size = 1.5)
    gg <- gg0 + 
    ...
    ## Interactive version
    ggint <- ggbase + geom_point_interactive(aes(tooltip = pid), 
                                             fill = "lightgrey", color = "grey", 
                                             pch = 21, size = 1.5) + 
        geom_point_interactive(data = tests[[nm]] %>% 
                                   dplyr::filter(.data[[volcind]]), 
                               aes(tooltip = pid), fill = "red", color = "grey",
                               pch = 21, size = 1.5)
    interactiveVolcanos[[nm]] <- girafe(ggobj = ggint)
    ...
    ## MA plot
    ggma <- ggplot(tests[[nm]], aes(x = .data[[xvma]], y = .data[[yvma]])) + 
        geom_hline(yintercept = 0, color = "black", linetype = "dashed") + 
        geom_point(fill = "lightgrey", color = "grey", pch = 21, size = 1.5) + 
        theme_bw() + coord_cartesian(ylim = xr) + 
        theme(axis.text = element_text(size = 12),
              axis.title = element_text(size = 14),
              title = element_text(size = 14)) + 
...
```{r linktable}
## linktype can be "UniProt", "AlphaFold", "WormBase" or "PomBase"
make_link_upid <- function(upid, linktype = "UniProt") {
    if (upid != "") {
        if (linktype == "UniProt") {
            ...
            sprintf('<a href="%s" target="_blank"> %s</a>',
                    paste0("https://www.pombase.org/gene/", upid), upid)
        } else if (linktype == "WormBase") {
            sprintf('<a href="%s" target="_blank"> %s</a>',
                    paste0("https://wormbase.org/species/c_elegans/gene/", upid), upid)
        } else {
            ""
        }
    } else {
...
linkTable <- as.data.frame(rowData(qft[[1]])) %>%
    ...
    dplyr::mutate(UniProt = vapply(Accession, function(mpds) {
        paste(vapply(strsplit(mpds, ";")[[1]], function(mpd) {
            make_link_upid(mpd, linktype = "UniProt")
        }, ""), collapse = ";")
    }, "NA")) %>%
    dplyr::mutate(AlphaFold = vapply(Accession, function(mpds) {
        paste(vapply(strsplit(mpds, ";")[[1]], function(mpd) {
            make_link_upid(mpd, linktype = "AlphaFold")
        }, ""), collapse = ";")
    }, "NA")) 

if (species_common == "fission yeast") {
    ## Add PomBase links
    pbconv <- read.delim("/tungstenfs/groups/gbioinfo/sonechar/FMIgroups/Proteomics/identifier_mappings/20220125/PomBase2UniProt.tsv", header = FALSE) %>%
        setNames(c("PomBaseID", "UniProtID"))
    linkTable <- linkTable %>%
        dplyr::mutate(PomBase = vapply(Accession, function(mpds) {
            paste(vapply(strsplit(mpds, ";")[[1]], function(mpd) {
                pbid <- pbconv$PomBaseID[pbconv$UniProtID == mpd]
                if (length(pbid) != 0) {
                    paste(vapply(pbid, function(pb) {
                        make_link_upid(pb, linktype = "PomBase")
                    }, ""), collapse = ";")
                } else {
                    ""
                }
...
if (species_common == "roundworm") {
    ## Add WormBase links
    wbconv <- read.delim("/tungstenfs/groups/gbioinfo/sonechar/FMIgroups/Proteomics/identifier_mappings/20220125/CAEEL_6239_idmapping.dat", header = FALSE) %>%
        dplyr::filter(V2 %in% c("UniProtKB-ID", "WormBase")) %>%
        tidyr::pivot_wider(id_cols = V1, names_from = V2, values_from = V3, 
                           values_fn = list) %>% 
        tidyr::unnest(cols = c("UniProtKB-ID", "WormBase")) %>%
        dplyr::mutate(`UniProtKB-ID` = gsub("_CAEEL$", "", `UniProtKB-ID`))
    linkTable <- linkTable %>%
        dplyr::mutate(WormBase = vapply(Accession, function(mpds) {
            wbids <- unlist(lapply(strsplit(mpds, ";")[[1]], function(mpd) {
                wbconv$WormBase[wbconv$`UniProtKB-ID` == mpd | 
                                    wbconv$V1 == mpd]
            }))
            if (length(wbids) != 0 && length(setdiff(wbids, "")) != 0) {
                ...
                paste(vapply(wbids, function(wb) {
                    if (!is.na(wb)) {
                        make_link_upid(wb, linktype = "WormBase")
                    } else {
                        ""
                    }
                }, ""), collapse = ";")
