...
qft <- QFeatures::readQFeatures(pdFile, ecol = iCols, name = aName, 
                                sep = "\t", fnames = "Proteins.Unique.Sequence.ID")
qft
```
...
gName <- sapply(strsplit(rowData(qft[[aName]])$Gene.Symbol, "; "), .subset, 1)

## Extract the accession
accession <- sapply(strsplit(rowData(qft[[aName]])$Accession, "; "), .subset, 1)

# ## Find features with missing gene name and replace with the corresponding 
# ## majority protein ID
# idxna <- which(is.na(gName))
# gName[idxna] <- accession[idxna]
