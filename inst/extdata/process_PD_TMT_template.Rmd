...
pd_InputFiles <- read.delim(paste0(pdOutputFolder, pdResultName, 
                                   "_InputFiles.txt"), sep = "\t")
pd_StudyInformation <- read.delim(paste0(pdOutputFolder, pdResultName,
                                         "_StudyInformation.txt"), sep = "\t")
...
pdFile <- paste0(pdOutputFolder, pdResultName, "_Proteins.txt")
```
...
(iColsAll <- grep(iColPattern, names(read.delim(pdFile)), value = TRUE))
```
...
## TODO: Choose a column with unique identifiers for fnames
## Read MaxQuant output
qft <- QFeatures::readQFeatures(pdFile, ecol = iCols, name = aName, 
                                sep = "\t", fnames = "Gene.Symbol")
qft
```
