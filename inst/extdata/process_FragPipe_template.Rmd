...
```{r show-feature-names-after-filtering}
rbind(
    head(as.data.frame(rowData(sce)[, c("einprotId", "einprotGene", "einprotProtein",
                                        "einprotLabel", "IDsForSTRING")]),
         n = min(5, floor(nrow(sce) / 2))),
...
```{r plot-sa, fig.width = 12, fig.height = min(48, ceiling(length(testres$tests)/3) * 4 + 0.1), warning = FALSE, eval = (stattest == "limma"), echo = (stattest == "limma")}
for (i in seq.int(length(testres$tests) %/% 36 + 1)) {
    tmplist <- testres$tests[(seq_along(testres$tests) - 1) %/% 36 == (i - 1)]
...
abundanceExport <- lapply(structure(names(testres$tests), names = names(testres$tests)), function(nm) {
    testres$tests[[nm]] %>% dplyr::select(pid, showInVolcano, starts_with(aval))
})
...
```{r top-feature-sets, fig.height = 4 * length(testres$featureCollections) + 0.1, eval = (stattest != "none"), echo = (stattest != "none")}
...
linkTable <- makeDbLinkTable(
    df = as.data.frame(rowData(sce)) %>%
        ...
        dplyr::select("pid", "einprotProtein", "einprotLabel",
                      matches(linkTableColumns)), 
...
sce <- prepareFinalSCE(
    ...
    featureCollections = testres$featureCollections, expType = "FragPipe")
