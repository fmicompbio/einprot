...
settingsList <- list(
    ...
    "Assays(s) to use for exported values" = paste(assaysToExport, collapse = ", "), 
...
```{r def-aval}
if (any(assaysToExport %in% assayNames(sce))) {
    assaysToExport <- intersect(assaysToExport, assayNames(sce))
} else {
    assaysToExport <- aNames$assayInput
}
```
...
testres <- runTest(sce = sce, comparisons = comparisonList$comparisons,
                   ...
                   aName = assaysToExport, addAbundanceValues = TRUE, 
                   singleFit = singleFit,
...
for (nm in names(testres$tests)) {
    plots <- plotVolcano(
        ...
        abundanceColPat = assaysToExport,
...
abundanceExport <- makeAbundanceExport(testresList = testres$tests, 
                                       abundancePrefix = assaysToExport)
...
makeiSEEScript(iSEEScript = iSEEScript, sceFile = sceFile, 
               ...
               assayForHeatmaps = assaysToExport[1],
