...
bibliography: "`r system.file('extdata', 'einprot_bibliography.bib', package = 'einprot', lib.loc = '/tungstenfs/groups/gbioinfo/sonechar/FMIgroups/Proteomics/devel_lib_einprot')`"
---
...
rowData(sce)$Potential.contaminant <- ifelse(grepl("contam_", rowData(sce)$Protein.ID), "+", "")
rowData(sce)$Reverse <- ifelse(grepl("rev_", rowData(sce)$Protein.ID), "+", "")
...
if ("batch" %in% colnames(colData(sce))) {
    ...
    if (subtractBaseline) {
        assay(sce, assayForPlots) <- 
            getMatSubtractedBaseline(sce, assayName = assayForTests,
