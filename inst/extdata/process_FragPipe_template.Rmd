...
```{r fix-ids}
sce <- fixFeatureIds(
    sce, idCol = idCol, labelCol = labelCol,
...
for (cmp in comparisons) {
    ...
    if (speciesInfo$speciesCommon == "roundworm") {
        tests[[paste0(cmp[2], "_vs_", cmp[1])]]$WormBaseID <- 
            wbconv$WormBaseID[match(tests[[paste0(cmp[2], "_vs_", cmp[1])]][["einprotProtein"]],
                                    wbconv$UniProtID)]
    } else if (speciesInfo$speciesCommon == "fission yeast") {
        tests[[paste0(cmp[2], "_vs_", cmp[1])]]$PomBaseID <- 
            pbconv$PomBaseID[match(tests[[paste0(cmp[2], "_vs_", cmp[1])]][["einprotProtein"]],
                                   pbconv$UniProtID)]
    }
}
```
