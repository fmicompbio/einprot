...
If you are using the results from this report in published work, please cite 
@Soneson2023einprot, as well as the underlying packages that are used for the 
analysis (indicated in the text). Documentation of all `einprot` functions, 
as well as a vignette describing typical use cases in more detail, are 
available from [GitHub](https://fmicompbio.github.io/einprot/).

```{r get-basic-info}
## Get species info and define STRINGdb object
speciesInfo <- getSpeciesInfo(species)
if (expType %in% c("MaxQuant", "FragPipe", "DIANN", "Spectronaut", "SE") || 
...
if (expType == "MaxQuant") {
    ...
    tb <- c(list("Input file" = inFile), summaryInfo)
} else {
    stop("Unknown 'expType'")
}

makeTableFromList(c(list("einprot version" = packageVersion("einprot")), 
                    tb))
```

```{r diann-cmd, echo=FALSE, eval = (expType == "DIANN"), results="asis"}
cat("\n\n")
cat("<details>\n<summary><b>\nClick to see the DIA-NN command line (if available)\n</b></summary>\n")
cat("\n````\n")
if ("DIA-NN command" %in% names(dia)) {
    cat(dia[["DIA-NN command"]])
}
cat("\n````\n\n")
```
...
```{r settings-table}
settingsList <- list(
    "Filter names (SE)" = paste(names(filtersSE), collapse = ", "),
    "Imputation method" = imputeMethod,
    "Additional imputation arguments (names)" = names(imputeArgs),
    "Assays(s) to use for exported values" = paste(assaysForExport, collapse = ", "), 
    "Add abundance values" = addAbundanceValues,
    ...
    "Max complex similarity to plot" = maxComplexSimilarity,
    "Number of permutations" = nperm,
    "Random seed" = seed,
    "User-defined feature annotation columns" = paste(names(extraFeatureCols), collapse = ";"),
...
if (expType != "SE") {
    settingsList <- c(
        list("Include only samples (if applicable)" = paste(includeOnlySamples, 
                                                            collapse = ", "),
...
if (expType == "ProteomeDiscoverer") {
    ...
    if (inputLevel == "Proteins") {
        settingsList <- settingsList[!(names(settingsList) %in% 
                                           c("Modifications column", 
                                             "Exclude unmodified peptides",
                                             "Modifications to keep"))]
    }
}
if (expType %in% c("Spectronaut", "DIANN")) {
    settingsList <- c(settingsList, 
                      list("Filter names (DF)" = paste(names(filtersDF), collapse = ", ")))
}
if (expType == "SE") {
    settingsList <- c(settingsList,
                      list("Assay name" = aName))
}
...
```{r read-data}
if (expType == "SE") {
    sce <- as(readRDS(inFile), "SingleCellExperiment")
} else {
    if (expType == "MaxQuant") {
        ...
        tmp <- importDIANN(inFile = diannFile, fileType = diannFileType, 
                           ...
                           includeOnlySamples = includeOnlySamples,
                           excludeSamples = excludeSamples, filtersDF = filtersDF)
    } else if (expType == "Spectronaut") {
        ## Read Spectronaut output
        tmp <- importSpectronaut(inFile = spectronautFile, 
                                 fileType = spectronautFileType, outLevel = outLevel, 
                                 iColPattern = iColPattern, 
                                 stopIfEmpty = FALSE, aName = aName, 
                                 includeOnlySamples = includeOnlySamples, 
                                 excludeSamples = excludeSamples,
...
```{r add-sampleinfo, eval=(expType != "SE"), echo=(expType != "SE")}
...
corresponds to. The second column contains the actual assay names. Note that 
the same actual assay name can correspond to multiple stages - this is the 
case, e.g., if normalization is skipped, or if no batch variable is supplied.
...
# Filter out contaminants, decoy hits, and features with low confidence

Next, we filter out any features classified as potential 
contaminants or decoy hits, and features identified only by site (which 
...
sce <- filterFeaturesSE(sce = sce, filtersSE = filtersSE, plotUpset = TRUE, 
...
if (length(extraFeatureCols) > 0) {
    sce <- fixFeatureIds(sce, colDefs = extraFeatureCols)
}
```
...
rbind(
    head(as.data.frame(rowData(sce)[, c("einprotId", "einprotGene", "einprotProtein",
                                        ...
                                        names(extraFeatureCols))]),
         n = min(5, floor(nrow(sce) / 2))),
    rep("...", 5 + length(extraFeatureCols)),
...
assay(sce, aNames$assayInput)[!is.finite(assay(sce, aNames$assayInput))] <- NA
...
if (normMethod == "vsn") {
    # natural-scale input -> log-scale output
    sce <- doNormalization(sce, method = normMethod, 
                           assayName = aNames$assayInput,
                           normalizedAssayName = aNames$assayLog2NormWithNA, 
                           spikeFeatures = spikeFeatures)
} else {
    # center.median, center.mean, diff.median, center.mean.shared, 
    # center.median.shared, diff.mean.shared, diff.median.shared
    # log-scale input -> log-scale output
    sce <- doNormalization(sce, method = normMethod, 
...
makeIntensityBoxplots(sce = sce, assayName = aNames$assayLog2NormWithNA,
                      ...
                      maxNGroups = 25)
if (normMethod %in% c("center.mean.shared", "center.median.shared",
                      "diff.mean.shared", "diff.median.shared")) {
    # plot distribution of shared features
    sharedFeats <- which(rowSums(is.na(assay(sce, aNames$assayLog2NormWithNA))) == 0)
    if (length(sharedFeats) > 0) {
        makeIntensityBoxplots(sce = sce[sharedFeats, ], assayName = aNames$assayLog2NormWithNA,
                              doLog = FALSE, 
                              ylab = paste0(aNames$assayLog2NormWithNA, " (",
                                            length(sharedFeats), " shared features)"), 
                              maxNGroups = 25)
    }
}
```

```{r text-filter-by-mod, results="asis", echo=FALSE, eval=(expType == "ProteomeDiscoverer" && inputLevel == "PeptideGroups")}
cat(filterByModText(excludeUnmodifiedPeptides = excludeUnmodifiedPeptides,
                    keepModifications = keepModifications))
```

```{r filter-by-mod}
if (expType == "ProteomeDiscoverer" && inputLevel == "PeptideGroups") {
    ## Filter by modification type
    sce <- filterFeaturesSE(
        sce = sce, 
        filtersSE = list(
            ExclUnmod = function(se) {
                if (modificationsCol %in% colnames(rowData(sce)) && 
                    excludeUnmodifiedPeptides) {
                    rowData(sce)[[modificationsCol]] != "" & 
                        !is.na(rowData(sce)[[modificationsCol]])
                } else {
                    rep(TRUE, nrow(sce))
                }
            }, 
            KeepMods = function(se) {
                if (modificationsCol %in% colnames(rowData(sce)) && 
                    !is.null(keepModifications)) {
                    grepl(keepModifications, rowData(sce)[[modificationsCol]]) & 
                        !is.na(rowData(sce)[[modificationsCol]])
                } else {
                    rep(TRUE, nrow(sce))
                }
        ...
        plotUpset = TRUE, 
        exclFile = sub("\\.Rmd$", paste0("_filtered_out_features_modifications.txt"),
                       knitr::current_input(dir = TRUE)))
}
```

At this stage, the `sce` object contains `r nrow(sce)` features.
...
if (addHeatmaps) {
    plotMissingValuesHeatmap(sce, assayMissing = aNames$assayImputIndic, 
                             ...
                             settings = ifelse(nrow(sce) <= 10000, "clustered", "sorted"))
}
```
...
set.seed(seed)
sce <- do.call(doImputation, 
               c(list(sce = sce, imputeMethod = imputeMethod,
...
plotImputationDistribution(sce, assayToPlot = aNames$assayImputed, 
                           ...
                           xlab = aNames$assayImputed, plotType = "histogram")
```
...
for (nm in names(testres$tests)) {
    plots <- plotVolcano(
        ...
        xv = NULL, yv = NULL, xvma = NULL, volcind = NULL, pv = NULL,
        ...
        abundanceColPat = abundanceColPat,
...
```{r text-coll, results="asis", echo=FALSE}
cat(featureCollectionText(featureCollections = names(testres$featureCollections)))
```
...
for (nm in names(testres$topsets)) {  ## for each comparison
    if (length(testres$topsets[[nm]]) > 0) {
        plts <- lapply(names(testres$topsets[[nm]]), function(snm) {  ## set type
            df <- testres$topsets[[nm]][[snm]]
            # if (nrow(df) > maxNbrComplexesToPlot) {
            #     df <- df[seq_len(maxNbrComplexesToPlot), ]
            # }
            if (nrow(df) > 0) {
                kp <- getComplexesToPlot(testres$featureCollections[[snm]], 
                                         complexCandidates = df$set, 
                                         maxNbrComplexesToPlot = maxNbrComplexesToPlot,
                                         maxComplexSimilarity = maxComplexSimilarity)
                df <- df[match(kp, df$set), , drop = FALSE]
...
[BioGRID](https://thebiogrid.org/) pages (as well as selected 
...
if (addHeatmaps) {
    ht <- makeAbundanceHeatmap(sce, assayToPlot = aNames$assayImputed, doCenter = FALSE, 
                               settings = "report", clusterRows = (nrow(sce) <= 10000))
...
```{r save-heatmap, message=FALSE, results="hide"}
...
if (addHeatmaps) {
    ...
    ht <- makeAbundanceHeatmap(sce, assayToPlot = aNames$assayImputed, doCenter = TRUE,
                               settings = "export", clusterRows = (nrow(sce) <= 10000))
...
hmFeature <- rownames(sce)[
    min(which(rowSums(!is.na(assay(sce, assaysForExport[1]))) > 0))]
