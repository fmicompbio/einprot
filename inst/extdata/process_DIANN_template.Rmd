...
quantified with [DIA-NN](https://github.com/vdemichev/DiaNN) [@Demichev2020diann]. 
...
makeTableFromList(dia[names(dia) != "DIA-NN command"])
```
...
The input to this workflow is a `pg_matrix.tsv` file from DIA-NN 
(see path in the table above). We read the DIA-NN intensities into `R` 
and store them in a 
...
types of intensities and annotations from the DIA-NN file.
...
from the column names in the `DIA-NN` file. The `group` 
...
Next, we filter out any features classified by DIA-NN as potential 
...
sce <- prepareFinalSCE(
    ...
    featureCollections = testres$featureCollections, expType = "DIANN")
