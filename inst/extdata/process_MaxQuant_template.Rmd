...
are shown, for interpretability reasons).
...
UpSetR::upset(tmpsign, nsets = length(tests), nintersects = 50, order.by = "freq")
```
...
ordr <- order(sce$group)
ht <- ComplexHeatmap::Heatmap(
    t(scale(t(assay(sce, assayForTests)[, ordr]), center = TRUE, scale = FALSE)),
    name = paste0(assayForTests, "\ncentered"),
    show_row_names = TRUE, show_row_dend = FALSE,
    cluster_columns = FALSE, use_raster = TRUE,
    top_annotation = columnAnnotation(group = sce$group[ordr],
                                      col = list(group = groupcols)),
    bottom_annotation = columnAnnotation(group = sce$group[ordr],
                                         col = list(group = groupcols)),
