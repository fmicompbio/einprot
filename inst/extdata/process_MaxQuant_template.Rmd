...
```{r get-submission-list, echo=FALSE}
...
```{r read-exp-info, echo=FALSE, warning=FALSE}
...
speciesInfo <- getSpeciesInfo(ei_species)
...
```{r define-complexdb-path, echo=FALSE}
...
if ("complexes" %in% includeFeatureCollections) {
    ...
    if (speciesInfo$speciesCommon %in% c("human", "mouse", "baker's yeast", 
...
```{r get-mq-info, echo=FALSE}
## Read MaxQuant XML file
mqPars <- readMaxQuantXML(mqParameterFile)
```
...
```{r mq-table, echo=FALSE}
makeTableFromList(mqPars)
```
...
if (speciesInfo$speciesCommon == "fission yeast") {
    ## Add PomBase links
    ...
    linkTable <- linkTable %>%
        dplyr::mutate(PomBase = vapply(Majority.protein.IDs, function(mpds) {
            paste(vapply(strsplit(mpds, ";")[[1]], function(mpd) {
...
if (speciesInfo$speciesCommon == "roundworm") {
    ...
    linkTable <- linkTable %>%
        dplyr::mutate(WormBase = vapply(Majority.protein.IDs, function(mpds) {
