...
subtitle: "`r paste0('Generated with einprot v0.1.0')`"
...
submission_list <- "/tungstenfs/groups/gbioinfo/sonechar/FMIgroups/Proteomics/seebajan/SampleSubmissions.xlsx"
...
suppressPackageStartupMessages({
    ...
    library(iSEE)
    # library(iSEEu)
    devtools::load_all('/tungstenfs/groups/gbioinfo/sonechar/Rpackages/iSEEu/iSEEu')
    library(limma)
    library(ggrepel)
    library(rrcovNA)
    library(circlize)
...
This report describes a reproducible end-to-end analysis of a protein dataset quantified 
...
user <- paste0(experiment_info$OwnerFullName, " (", 
               experiment_info$OwnerUserName, ")")
group <- experiment_info$GroupName
operator <- experiment_info$Operator
submission_date <- as.character(as.Date(experiment_info$Created))
project_name <- experiment_info$ProjectName
project_id <- experiment_info$ProjectID
experiment_name <- experiment_info$ExperimentName
project_description <- experiment_info$ProjectDescription
experiment_description <- experiment_info$ExperimentDescription
sample_type <- experiment_info$SampleType
nbr_samples <- experiment_info$NoSamples
species <- experiment_info$Species
if (species == "C.elegans") {
    species <- "Caenorhabditis elegans"
}
type_of_analysis <- experiment_info$MasterTemplateName
```
...
if (tolower(species) %in% tolower(taxTable$species_common)) {
    species_id <- taxTable$species[match(tolower(species),
                                         tolower(taxTable$species_common))]
    species_common <- taxTable$species_common[match(tolower(species),
                                                    tolower(taxTable$species_common))]
...
if ("complexes" %in% includeFeatureCollections) {
    ## Currently only supported for human, mouse and yeast
    if (species_common %in% c("human", "mouse", "baker's yeast", 
                              "fission yeast", "roundworm")) {
            complexes_file <- "/tungstenfs/groups/gbioinfo/sonechar/FMIgroups/Proteomics/complex_db/complexdb_20211026_orthologs.rds"
    } else {
...
kableExtra::kbl(tbl) %>%
    ...
    kableExtra::column_spec(column = 1, width = "25em", bold = TRUE, border_right = TRUE) %>%
...
```{r, echo=FALSE}
mq_tbl <- data.frame(c2 = c("MaxQuant version" = mq_version, 
...
```{r, echo = FALSE}
tbl <- data.frame(c2 = c("Name of base assay" = aName,
                         ...
                         "Include only samples (if applicable)" = includeOnlySamples,
                         "Exclude samples" = excludeSamples,
...
kableExtra::kbl(tbl) %>%
    kableExtra::kable_paper(full_width = TRUE, lightable_options = c("striped"),
                            html_font = "\"Trebuchet MS\", verdana, sans-serif") %>%
    kableExtra::column_spec(column = 1, width = "100em", bold = TRUE, border_right = TRUE) %>%
    kableExtra::column_spec(column = 2, width = "100em")
```
...
if ("complexes" %in% includeFeatureCollections) {
    ...
    if (species_common %in% names(complexes)) {
        ...
        stop("Invalid species for testing complexes")
    }
    ...
    mcols(crl)$genes <- sapply(crl, 
                               function(w) gsub(pat, "\\1; ", paste(w, collapse = ";")))
} else {
    crl <- CharacterList()
}
...
if (length(crl) > 0) {
    ## Remove duplicate collections (for now based only on the name)
    crl <- crl[!duplicated(names(crl))]
...
above, the following samples are retained: 
...
gName <- sapply(strsplit(rowData(qft[[aName]])$Gene.names, ";"), .subset, 1)

## Extract the first annotated majority protein ID
majProtID <- sapply(strsplit(rowData(qft[[aName]])$Majority.protein.IDs, ";"), .subset, 1)
...
## TODO: Modify feature collections so that they contain any row name 
## corresponding to the features in the collection
...
Next we consider the overall distribution of the imputed log2-intensities among 
the samples (after imputation). 
...
We also plot the mean-variance relationship of the imputed log2-intensities.
...
For each feature, we then compare the (imputed) log2 intensities between each 
group and the designated control group. 
...
cat(paste0("For this, we use a Student's t-test. To determine which features ",
           ...
           "camera method [@Wu2012camera] to test for significance of each ", 
           "included feature collection. These tests are based on the ",
           "SAM statistic calculated from the t-statistic and the desired S0."))
```

The `r assayForTests` assay will be used for the tests. We will perform the 
...
for (cmp in comparisons) {
    ...
    ibaq_values <- as.data.frame(
        rowData(qftsub[[aName]])[, sub(iColPattern, "iBAQ.", colnames(qftsub[[aName]]))]
    )
    ...
    for (gr in unique(colData(qftsub)$group)) {
        ...
        res[[paste0("log2_iBAQ.", gr, ".sd")]] <- 
            genefilter::rowSds(log_ibaq_values[, colData(qftsub)$group == gr, drop = FALSE],
                               na.rm = TRUE)
    }
                ...
                if (length(intersect(prs, tests[[nm]]$pid)) > 1) {
                    gg <- gg0 + 
                        ...
                        geom_text_repel(data = tests[[nm]] %>% 
                                        ...
                                        aes(label = .data$pid), max.overlaps = Inf, size = 4,
                                        min.segment.length = 0.1) + 
                        labs(caption = paste0(cplx, ", PValue = ", cplxpval, ", FDR = ", cplxfdr))
                    print(gg)
                    ...
                    print(
                        ggplot(bardata %>% dplyr::group_by(pid, group) %>%
                            ...
                            geom_jitter(data = bardata, aes(y = iBAQ), 
                                        position = position_dodge(width = 0.9)) + 
                            theme_bw() + 
                            theme(axis.text = element_text(size = 12),
                                  axis.title = element_text(size = 14),
...
(defined as those colored in the volcano plots above).
