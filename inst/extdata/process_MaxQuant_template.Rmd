...
transformed and imputed quantifications. 
...
assay(sce, paste0("log2_", aName))[!is.finite(assay(sce, paste0("log2_", aName)))] <- NA
...
set.seed(seed)
assay(sce, paste0("log2_", aName)) <- MsCoreUtils::impute_matrix(
    assay(sce, paste0("log2_", aName)), method = "MinProb"
)
```

```{r impute-impseqrob, eval=(imputeMethod=="impSeqRob"), echo=(imputeMethod=="impSeqRob")}
set.seed(seed)
tmp <- impSeqRob(assay(sce, paste0("log2_", aName)))
assay(sce, paste0("log2_", aName)) <- tmp$x
```
...
```{r mean-var, message=FALSE}
makeMeanSDPlot(sce = sce, assayName = paste0("log2_", aName),
               xlab = "Mean log2 intensity (after imputation)",
               ylab = "SD of log2 intensity (after imputation)")
```


```{r text-no-norm, results="asis", echo=FALSE, eval=(normMethod == "none")}
cat(paste0("The log2 intensities are not normalized further across samples ", 
           "since 'normMethod' is set to 'none'."))
```

```{r text-norm, results="asis", echo=FALSE, eval=(normMethod != "none")}
...
makeMeanSDPlot(sce = sce, assayName = paste0("log2norm_", aName), 
...
for (cmp in comparisons) {
    outres <- runTest(sce = sce, comparison = cmp, testType = stattest, 
                      ...
                      volcanoS0 = volcanoS0, aName = aName, addAbundanceValues = TRUE)
    ...
    colpats[[paste0(cmp[2], "_vs_", cmp[1])]] <- outres$colpat
    topsets[[paste0(cmp[2], "_vs_", cmp[1])]] <- outres$topSets
...
abundanceExport <- lapply(structure(names(tests), names = names(tests)), function(nm) {
    tests[[nm]] %>% dplyr::select(pid, showInVolcano, starts_with(colpats[[nm]]))
})
...
collections, for each comparison. 
