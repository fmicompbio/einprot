...
```{r get-icols-all}
ics <- getIntensityColumns(inFile = mqFile, iColPattern = iColPattern,
...
```{r read-data}
## Read MaxQuant output
qft <- QFeatures::readQFeatures(mqFile, ecol = ics$iCols, name = aName, sep = "\t")
...
```{r fix-ids}
qft <- fixFeatureIds(qft, geneIdCol = "Gene.names",
...
plotMissingValuesHeatmap(qft, assayMissing = paste0("imputed_", aName))
```
