...
colData(sce)$nNA2 <- colSums(is.na(assay(sce, aNames$assayInput)))
colData(sce)$pNA2 <- sce$nNA2/nrow(sce)
rowData(sce)$nNA2 <- rowSums(is.na(assay(sce, aNames$assayInput)))
rowData(sce)$pNA2 <- rowData(sce)$nNA2/ncol(sce)
...
plotFractionDetectedPerSample(dfNA = nbr_na$nNAcols, aName = aNames$assayInput)
# plotFractionDetectedPerSample(dfNA = colData(sce)[, c("sample", "nNA2", "pNA2")])
```
...
plotDetectedInSamples(dfNA = nbr_na$nNArows, aName = aNames$assayInput)
# plotDetectedInSamples(dfNA = rowData(sce)[, c("nNA2", "pNA2")])
```
...
assay(sce, aNames$assayLog2NormWithNA) <- 
    MsCoreUtils::normalize_matrix(assay(sce, aNames$assayLog2WithNA),
                                  method = normMethod)
