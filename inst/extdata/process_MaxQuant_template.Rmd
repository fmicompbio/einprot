...
# Generate table with direct database links

```{r}
make_link_uniprot <- function(upid) {
    if (upid != "") {
...
linkTable <- as.data.frame(rowData(qft[[1]])) %>%
    tibble::rownames_to_column("pid") %>%
    dplyr::select(pid, Majority.protein.IDs, IDsForSTRING) %>%
    dplyr::mutate(Majority.protein.IDs = vapply(Majority.protein.IDs, function(mpds) {
        paste(vapply(strsplit(mpds, ";")[[1]], function(mpd) {
            make_link_uniprot(mpd)
        }, ""), collapse = ";")
    }, "NA"))
