...
```{r plot-sa, fig.width = 12, fig.height = min(48, ceiling(length(testres$tests)/3) * 4 + 0.1), warning = FALSE, eval = (testType == "interaction"), echo = (testType == "interaction")}
...
linkTable <- makeDbLinkTable(
    df = as.data.frame(rowData(scePeptides)) %>%
        ...
        dplyr::select("pid", "Peptides_einprotProtein", 
                      matches(setdiff(c("^Peptides_einprotLabel$", linkTableColumns), ""))) %>%
