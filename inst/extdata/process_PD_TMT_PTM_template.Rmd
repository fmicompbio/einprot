...
```{r run-test, echo = (testtype == "interaction"), eval = (testtype == "interaction")}
...
```{r run-test, echo = (testtype == "welch"), eval = (testtype == "welch")}
testres <- runWelchPTMTest(sceProteins = sceProteins, 
                           scePeptides = scePeptides, 
                           comparisons = comparisonList$comparisons, 
                           volcanoAdjPvalThr = volcanoAdjPvalThr, 
                           volcanoLog2FCThr = volcanoLog2FCThr)
...
```{r plot-sa, fig.width = 12, fig.height = ceiling(length(tests)/3) * 4, warning = FALSE, eval = (testtype == "interaction"), echo = (testtype == "interaction")}
...
```{r volcano-limma, warning=FALSE, fig.width=10.5, fig.height=7.5}
...
for (nm in names(tests)) {
    plots <- plotVolcano(
        sce = scePeptides, res = tests[[nm]], testType = "limma", 
