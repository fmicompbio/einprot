...
```{r text-sa, results="asis", echo=FALSE, eval = (testType == "interaction"), echo = (testType == "interaction")}
...
for (nm in names(tests)) {
    plots <- plotVolcano(
        sce = scePeptides, res = tests[[nm]], testType = testType, 
...
all_tests$pid <- NULL
colnames(all_tests) <- paste0(colnames(all_tests), "_PTM")
...
for (comparisonName in names(comparisonList$comparisons)) {
    if (all(c(paste0("Proteins_", comparisonName, ".logFC"),
              ...
              paste0(comparisonName, ".showInVolcano_PTM")) %in% 
        ...
        cv <- paste0(comparisonName, ".showInVolcano_PTM")
